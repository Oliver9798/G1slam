...
private:
    ...
    /// @brief sub baselink2odom,(fast_lio odom)
    ros::Subscriber sub_baselink2odom_;

    /// @brief  sub current pointcloud scan
    ros::Subscriber sub_scan_cur_;

    /// @brief init pose
    ros::Subscriber sub_initialpose_;

    /// @brief baselink2odom pose
    ...
    /// @brief baselink2map = mat_odom2map * mat_baselink2odom
    ...
    /// @brief baselink2motionlink
    Eigen::Matrix4d mat_baselink2motionlink_;

    /// @brief imulink2baselink
    Eigen::Matrix4d mat_imulink2baselink_;

    /// @brief init post x, y, z, roll, pitch, yaw (degrees)
    ...
    /// @brief locolization frequency(s)
    double loc_frequence_;

    /// @brief source max pointcloud
    int maxpoints_source_ = 50000;
    /// @brief target max pointcloud 
...
void GloabalLocalization::LocalizationInitialize()
{
    ...
    /// baselink2odom(camera_init) and map
    ...
    double loc_cost = 0;    /// loc time (ms)
    int count_success = 0;
    while (1)
    {
        auto loc_s = std::chrono::high_resolution_clock::now(); /// start
        lock_scan_.lock();
        ...
        else
        {
            ...
            *pcd_scan = *pcd_scan_cur_;
            ...
            /// transfer cropbox to current location and crop 
            OBB_map->center_ = mat_baselink2map_cur.block<3, 1>(0, 3);   // cur2map position
            OBB_map->R_ = mat_baselink2map_cur.block<3, 3>(0, 0);        // cur2map rotation
            OBB_scan->center_ = mat_baselink2odom_cur.block<3, 1>(0, 3); // cur2odom position
            OBB_scan->R_ = mat_baselink2odom_cur.block<3, 3>(0, 0);      // cur2odom rotation
